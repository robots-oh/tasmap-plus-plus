...
def filter_point(point_frame_matrix, node, pcld_list, point_ids_list, mask_point_clouds, frame_list, point_filter_threshold):
    '''
...
def dbscan_process(pcld, point_ids, DBSCAN_THRESHOLD=0.1):
    ...
    for i in range(1,len(count)):
        ...
        if len(remain_index) == 0:
            continue
        if len(remain_index) < 10:  # 작은 거 버리기
            continue
...
def export_class_agnostic_mask(class_agnostic_mask_list, save_root_path):
    ...
    np.savez(os.path.join(save_root_path, 'class_agnostic_mask.npz'), **pred_dict)
    return


def export(dataset, total_point_ids_list, total_mask_list, save_root_path):
    '''
    ...
    export_class_agnostic_mask(class_agnostic_mask_list, save_root_path)

    os.makedirs(save_root_path, exist_ok=True)
    np.save(os.path.join(save_root_path, 'object_dict.npy'), object_dict, allow_pickle=True)

import open3d as o3d
def filter_object_points(point_ids, scene_points, nb_neighbors=20, std_ratio=2.0):
    """
    특정 object(points)에 대해 noise를 제거하고, 남은 point_ids를 반환합니다.

    Args:
        point_ids (np.ndarray): 대상 object의 point 인덱스 (len = M)
        scene_points (np.ndarray): 전체 scene의 point cloud (N, 3)
        nb_neighbors (int): Statistical Outlier Removal에서 고려할 이웃 수
        std_ratio (float): 표준편차 허용 비율

    Returns:
        filtered_point_ids (np.ndarray): noise가 제거된 후 남은 원본 point_ids
    """
    # 1. points 가져오기
    points = scene_points[point_ids]

    # 2. points를 Open3D PointCloud로 변환
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points)

    # 3. Statistical Outlier Removal 적용
    _, inlier_indices = pcd.remove_statistical_outlier(nb_neighbors=nb_neighbors,
                                                       std_ratio=std_ratio)
    
    # 4. inlier에 해당하는 point_ids만 반환
    filtered_point_ids = point_ids[inlier_indices]

    return filtered_point_ids

def filter_multiple_objects_by_statistical(total_point_ids_list, scene_points, nb_neighbors=10, std_ratio=2.0):
    """
    여러 object point_ids를 받아 각각 Statistical Outlier Removal(SOR)로 noise를 제거합니다.

    Args:
        total_point_ids_list (List[np.ndarray]): object별 point_ids 리스트
        scene_points (np.ndarray): 전체 scene point cloud (N, 3)
        nb_neighbors (int): SOR에서 고려할 이웃 수
        std_ratio (float): 표준편차 허용 비율

    Returns:
        final_pcld_list (List[o3d.geometry.PointCloud]): noise 제거 후 PointCloud 리스트
        final_point_ids_list (List[np.ndarray]): noise 제거 후 point_ids 리스트
    """
    final_pcld_list = []
    final_point_ids_list = []

    for point_ids in total_point_ids_list:
        # noise 제거
        filtered_point_ids = filter_object_points(point_ids, scene_points, nb_neighbors, std_ratio)

        if len(filtered_point_ids) == 0:
            filtered_point_ids = point_ids

        # 남은 points로 새 PointCloud 만들기
        points = scene_points[filtered_point_ids]
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points)

        final_pcld_list.append(pcd)
        final_point_ids_list.append(filtered_point_ids)

    return final_pcld_list, final_point_ids_list

def post_process(dataset, node_list, mask_point_clouds, scene_points, point_frame_matrix, frame_list, point_filter_threshold, save_root_path):
    ...
    for node in tqdm(node_list, total=len(node_list), desc="exporting"):
        ...
        point_ids_list, bbox_list, mask_list = filter_point(point_frame_matrix, node, pcld_list, point_ids_list, mask_point_clouds, frame_list, point_filter_threshold)
    ...
    _, total_point_ids_list = filter_multiple_objects_by_statistical(total_point_ids_list, scene_points)
    export(dataset, total_point_ids_list, total_mask_list, save_root_path)
    return