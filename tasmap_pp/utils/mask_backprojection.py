...
def backproject(depth, intrinisc_cam_parameters, extrinsics):
    """
    Returns pcld and 1D mask that aligns with resulting view_points
    """
    depth_mask = (depth > 0) & (depth <= DEPTH_TRUNC)

    depth_filtered = np.where(depth_mask, depth, 0).astype(np.float32)

    depth_o3d = o3d.geometry.Image(depth_filtered)
    pcld = o3d.geometry.PointCloud.create_from_depth_image(
        depth_o3d,
    ...
    return pcld, depth_mask.reshape(-1)
...
def get_depth_mask(depth):
    """
    Return a depth mask consistent with backproject().
    depth: np.ndarray of shape (H, W)
    """
    depth_mask = np.logical_and(depth > 0, depth <= DEPTH_TRUNC)
    return torch.from_numpy(depth_mask.astype(np.bool_)).reshape(-1).cuda()
...
def turn_mask_to_point(dataset, scene_points, mask_image, frame_id):
    ...
    colored_pcld, _ = backproject(depth, intrinisc_cam_parameters, extrinsics)
